...
import 'package:sistem_akademik/model/model_siswa.dart' as siswa;
import 'package:sistem_akademik/model/model_kelas.dart' as kelas;
...
class _AddSiswaScreenState extends State<AddSiswaScreen> {
  ...
  Future<void> _fetchKelas() async {
    final String apiUrl = "http://192.168.1.34:8080/sistem_akademik/get_kelas.php";
    try {
      final response = await http.get(Uri.parse(apiUrl));

      if (response.statusCode == 200) {
        final kelas.ModelKelas modelKelas = kelas.modelKelasFromJson(response.body);
        setState(() {
          kelasList = modelKelas.data;
          print('Data Kelas: ${kelasList.map((k) => k.kelas).toList()}'); // Log data kelas
        });
      } else {
  ...
  Future<void> _addSiswa() async {
    if (selectedKelas == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Mohon pilih Kelas")),
      );
      return;
    }

    final String apiUrl = "http://192.168.1.34:8080/sistem_akademik/add_siswa.php";
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          "id_kelas": selectedKelas ?? '',  // Menyediakan default value jika selectedKelas null
        }),
      );
          ...
          child: Column(
            children: <Widget>[
              ...
              DropdownButtonFormField<String>(
                ...
                value: selectedKelas,
                items: kelasList.isNotEmpty
                    ? kelasList.map<DropdownMenuItem<String>>((kelas.Datum kelas) {
                    ...
                    : [
                        DropdownMenuItem(child: Text("Tidak ada kelas"), value: null),
                      ],
                onChanged: (value) {
                  setState(() {
                    selectedKelas = value;
